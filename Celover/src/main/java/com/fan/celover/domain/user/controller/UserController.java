...
public class UserController {
	...
	// SNS 가입회원 추가정보입력 페이지
	@GetMapping("/visitor/additional-info")
	public String additionalInfoForm() {
		return "main/user/additionalInfoForm";
	}
	 ...
	 * public @ResponseBody String kakaoCallback(String code) { // code : 토큰 받기에 필요한
	 * 인가 코드 // return "카카오 인증 완료" + code;
	 * 
	 * // POST 방식으로 key-value 데이터를 요청
	 * 
	 * RestTemplate rt = new RestTemplate();
	 * 
	 * // HttpHeader 오브젝트 생성 HttpHeaders header = new HttpHeaders();
	 * 
	 * // request header 작성 header.add("Content-type",
	 * "application/x-www-form-urlencoded;charset=utf-8");
	 * 
	 * // HttpBody 오브젝트 생성 MultiValueMap<String, String> params = new
	 * LinkedMultiValueMap<>();
	 * 
	 * // request body 작성 params.add("grant_type", "authorization_code");
	 * params.add("client_id", "d15d0ec9bd2e7e5bd2904659955fbb98");
	 * params.add("redirect_uri", "http://localhost:8888/auth/kakao/callback");
	 * params.add("code", code);
	 * 
	 * // HttpHeader 와 HttpBody를 하나의 오브젝트로 합치기 HttpEntity<MultiValueMap<String,
	 * String>> entity = new HttpEntity<>(params, header);
	 * 
	 * ResponseEntity<String> response = rt.exchange(
	 * "https://kauth.kakao.com/oauth/token", HttpMethod.POST, entity, String.class
	 * // 응답데이터는 String 타입 );
	 * 
	 * //return "카카오 토큰 요청 완료" + response; // 이제 이 응답 데이터를 담을 수 있는 자바 객체를 만들어야한다.
	 * OAuthToken.java
	 * 
